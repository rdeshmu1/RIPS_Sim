...
Torque = 500; %P*A_cord*r_total_spool; %Nm
%torque of spool
...
%             
        ...
        T_cord(i) = Torque/r_total_spool; %r_new_spool(i);
       
        %finds changing T_cord
        Fd_parachute(i) = 0.5*rho_parachute(i)*Cd_parachute*v_parachute(i)^2*A_parachute;
        %new Fd parachute
        
        a_parachute(i) = (g-(Fd_parachute(i)/m_parachute)+(T_cord(i)/m_parachute));
        %new accelration
        
        v_parachute(i+1)=v_parachute(i)+deltat*a_parachute(i);
        %assume constant a
        
        x_parachute(i+1) = x_parachute(i) + v_parachute(i)*deltat + (a_parachute(i)*deltat^2)/2;
...
plot(time, cord)
xlabel('Time (s)')
ylabel('Length of cord deployed (m)')
