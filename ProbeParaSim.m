...
for i=1:z
   ...
   if current_time > 3600
    for l = 1:ripcord
        rho_parachute(l) = (3000/60000000)*x_parachute(l);
        T_cord = Torque/R_spool;
        Fd_parachute = T_cord;
        v_parachute(l) = sqrt((2*Fd_parachute)/(rho_parachute(l)*Cd_parachute*A_parachute));
        x_parachute(l) = x_probe_with_deployment(i) + l;
    end
   end
...
figure
plot(time, v_probe_with_deployment)
xlabel('Time (s)')
ylabel('Probe Velocity (m/s)')

figure
plot(time, x_probe_with_deployment)
xlabel('Time (s)')
